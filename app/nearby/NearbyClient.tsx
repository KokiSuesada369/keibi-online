...
const CITY_PLACEHOLDER: Record<string, string> = {
  ...
  '宮城県': '例：仙台市青葉区','秋田県': '例：秋田市','山形県': '例：山形市',
  '福島県': '例：福島市','茨城県': '例：水戸市','栃木県': '例：宇都宮市',
  '群馬県': '例：前橋市','埼玉県': '例：さいたま市大宮区','千葉県': '例：千葉市中央区',
  '東京都': '例：新宿区','神奈川県': '例：横浜市西区','新潟県': '例：新潟市中央区',
  '富山県': '例：富山市','石川県': '例：金沢市','福井県': '例：福井市',
  '山梨県': '例：甲府市','長野県': '例：長野市','岐阜県': '例：岐阜市',
  '静岡県': '例：静岡市葵区','愛知県': '例：名古屋市中区','三重県': '例：津市',
  '滋賀県': '例：大津市','京都府': '例：京都市中京区','大阪府': '例：大阪市北区',
  '兵庫県': '例：神戸市中央区','奈良県': '例：奈良市','和歌山県': '例：和歌山市',
  '鳥取県': '例：鳥取市','島根県': '例：松江市','岡山県': '例：岡山市北区',
  '広島県': '例：広島市西区','山口県': '例：山口市','徳島県': '例：徳島市',
  '香川県': '例：高松市','愛媛県': '例：松山市','高知県': '例：高知市',
...
const inputStyle: React.CSSProperties = {
  ...
  borderRadius: '8px', fontSize: '14px', color: '#111', background: '#fff',
  boxSizing: 'border-box', outline: 'none',
}
const labelStyle: React.CSSProperties = {
  fontSize: '13px', fontWeight: 700, color: '#444', marginBottom: '6px', display: 'block',
}
  ...
  const handleSearch = async () => {
    ...
    if (pref === '広島県') {
      const { data: saizen } = await supabase
        .from('companies').select('slug, name, pref, city, tel, numbers')
        .eq('slug', 'saizen-666').single()
      if (saizen) companies = [saizen, ...companies.filter(c => c.slug !== 'saizen-666')]
    }
    ...
    <div style={{ minHeight: '100vh', background: '#f8f9fa' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '1.25rem', marginBottom: '16px' }}>
          <div style={{ fontSize: '15px', fontWeight: 'bold', color: '#111', marginBottom: '14px' }}>📍 エリアを選択</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <div>
              ...
              <input
                ...
                style={inputStyle} onKeyDown={e => { if (e.key === 'Enter') handleSearch() }}
              />
            </div>
          </div>
        </div>
        ...
        <button onClick={handleSearch} disabled={loading} style={{
          ...
          fontWeight: 'bold', cursor: loading ? 'not-allowed' : 'pointer', marginBottom: '24px',
        }}>
          {loading ? '検索中...' : 'この条件で警備会社を探す'}
        </button>
        {loading && (
          <div style={{ textAlign: 'center', padding: '2rem 0' }}>
            <div style={{ display: 'inline-block', width: '36px', height: '36px', border: '3px solid #e5e7eb', borderTop: '3px solid #f97316', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
        ...
        {!loading && searched && results !== null && (
          <div>
            <div style={{ fontSize: '13px', color: '#666', marginBottom: '12px' }}>
              {results.length > 0 ? `${pref}${city ? `・${city}` : ''}の警備会社 ${results.length}件` : ''}
            </div>
            {results.length === 0 ? (
              <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '2rem', textAlign: 'center', color: '#888', fontSize: '14px' }}>
                該当する警備会社が見つかりませんでした
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {results.map(c => (
                  <div key={c.slug} style={{ background: '#fff', borderRadius: '10px', border: '1px solid #e5e7eb', padding: '14px 16px' }}>
                    <a href={`/companies/${c.slug}`} style={{ fontSize: '15px', fontWeight: 700, color: '#1a1a2e', textDecoration: 'none' }}>{c.name}</a>

      ...
      <section style={{ position: 'relative', padding: '56px 24px 48px', textAlign: 'center', background: '#0f172a', overflow: 'hidden' }}>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          (function(){
            ...
            var ps=[];
            for(var i=0;i<60;i++)ps.push({x:Math.random()*c.width,y:Math.random()*c.height,r:Math.random()*1.5+0.3,vx:(Math.random()-0.5)*0.4,vy:(Math.random()-0.5)*0.4,a:Math.random()*0.5+0.1});
            function draw(){
              ...
              for(var i=0;i<ps.length;i++){
                ...
                for(var j=i+1;j<ps.length;j++){
                  var q=ps[j];var dx=p.x-q.x;var dy=p.y-q.y;var d=Math.sqrt(dx*dx+dy*dy);
                  if(d<80){x.beginPath();x.moveTo(p.x,p.y);x.lineTo(q.x,q.y);x.strokeStyle='rgba(255,255,255,'+(0.08*(1-d/80))+')';x.lineWidth=0.5;x.stroke();}
                }
              }
